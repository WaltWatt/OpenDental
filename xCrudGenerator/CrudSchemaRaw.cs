...
namespace xCrudGenerator {
	...
	public class CrudSchemaRaw {
		...
		public static string AddTable(string tableName,List<DbSchemaCol> cols,int tabInset,bool isMobile,bool doRunQueries) {
			...
			strb.Append(rn+tb+t1+"command=@\"");
			sbCommand.Append("CREATE TABLE "+tableName+" (");
			for(int i=0;i<cols.Count;i++) {
				sbCommand.Append(rn+tb+t2+cols[i].ColumnName+" "+GetMySqlType(cols[i]));
			...
			for(int i=0;i<indexes.Count;i++) {
				...
				sbCommand.Append(rn+tb+t2+"INDEX("+indexes[i].ColumnName+")");
			}
			sbCommand.Append(rn+tb+t2+") DEFAULT CHARSET=utf8");
			if(doRunQueries) {
...
//				}
		...
		public static string AddColumnEnd(string tableName,DbSchemaCol col,int tabInset,bool doRunQuery) {
			StringBuilder strb = new StringBuilder();
			string command;
			tb="";//must reset tabs each time method is called
			for(int i=0;i<tabInset;i++){//defines the base tabs to be added to all lines
			...
			command="ALTER TABLE "+tableName+" ADD "+col.ColumnName+" "+GetMySqlType(col)+(col.DataType==OdDbType.DateTimeStamp ? "" : " NOT NULL")+defaultVal;
			strb.Append(rn+tb+t1+"command=\""+command+"\";");
//			strb.Append(rn+tb+t1+"//If ColEnd might be over 65k characters, use mediumtext");
			...
			if(col.DataType==OdDbType.Long) {//key or foreign key
				command="ALTER TABLE "+tableName+" ADD INDEX ("+col.ColumnName+")";
				if(doRunQuery) {
					DataCore.NonQ(command);
				}
				strb.Append(rn+tb+t1+"command=\""+command+"\";");
