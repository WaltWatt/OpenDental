...
namespace xCrudGenerator {
	public class CrudGenHelper {
		...
		public static void ConnectToDatabase(string dbName,string server="localhost",string user="root",string password=""){
			if(server.ToLower()=="server" && dbName.ToLower()=="customers") {
		...
		public static void AddMissingPreferences(string convertDbFileName,string logFileName="") {
			...
			foreach(PrefName pref in listPrefs) {
				...
				Regex insertRegex=new Regex(@"INSERT INTO preference(\s*)\(PrefName,(\s*)ValueString\) VALUES(\s*)\('"+pref.ToString()+"'");
				...
				string prefCommand=prefCommandLine.Trim().Substring("command=\"".Length);
				int endOfCommand=prefCommand.IndexOf("')\";")+2;
				if(endOfCommand==1) {
					endOfCommand=prefCommand.IndexOf(")\";")+1;//Eg. command="INSERT INTO preference(PrefName,ValueString) VALUES('RadiologyDateStartedUsing154',"+POut.DateT(DateTime.Now.Date)+")";
				}
		...
		public static void UpdateDatabase(string database,string server,string user,string password) {
			...
			string logFileName="CrudGeneratorLog.txt";
			try {
				...
				foreach(Type typeClass in assembly.GetTypes()) {
					if(typeClass.IsSubclassOf(typeTableBase) && !IsMobile(typeClass)) {
						CrudQueries.Write(convertDbFile,typeClass,database,false,true,false,logFileName);
					}
				}
