...
namespace xCrudGenerator {
	public partial class Form1:Form {
		...
		public static string convertDbFile=@"..\..\..\OpenDentBusiness\Misc\ConvertDatabases5.cs";
		public static string convertDbFilem=@"..\..\..\OpenDentBusiness\Mobile\ConvertDatabasem.cs";
		...
		private void butRun_Click(object sender,EventArgs e) {
			...
			if(checkRun.Checked){
				...
				for(int i=0;i<tableTypes.Count;i++){
					...
					string crudFileName=Path.Combine(crudDir,className+".cs");
					bool isNewFile=!File.Exists(crudFileName);
					File.WriteAllText(crudFileName,strb.ToString());
					if(isNewFile) {
						CrudGenHelper.AddToSubversion(Path.Combine(Application.StartupPath,crudDir,className+".cs"));
						ListFilesToAddToODBiz.Add(Path.Combine("Crud",Path.GetFileName(crudFileName)));
					}
					CrudQueries.Write(convertDbFile,tableTypes[i],textDb.Text,false,checkRunQueries.Checked,checkAddToConvertScript.Checked);
			...
			if(checkRunM.Checked) {
				...
				for(int i=0;i<tableTypesM.Count;i++) {
					...
					CrudQueries.Write(convertDbFilem,tableTypesM[i],textDbM.Text,true,checkRunQueries.Checked,checkAddToConvertScript.Checked);
...
//Todo: ReplicationServers.GetKey() needs to work for mobile.  Not needed until we start inserting records from mobile.
			...
			if(CrudGenHelper.HasBatchWriteMethods(typeClass)) {
				...
				strb.Append(rn+t4+"}");
				strb.Append(rn+t3+"}");
				strb.Append(rn+t3+"else {");//MySQL and not using Random Primary Keys.
				...
				strb.Append(rn+t5+"if(sbCommands.Length+sbRow.Length+1 > TableBase.MaxAllowedPacketCount) {");//plus 1 for comma
			...
			for(int f=0;f<fieldsExceptPri.Count;f++) {
				...
				if(!fieldsExceptPri.Skip(f+1).All(x => CrudGenHelper.GetSpecialType(x).HasAnyFlag(
