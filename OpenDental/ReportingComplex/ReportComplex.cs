...
namespace OpenDental.ReportingComplex {
	...
	public class ReportComplex {
		...
		///<summary>Invoke this action in order to close the report progress window.  Gets instantiated in the constructor.</summary>
		private Action _actionCloseReportProgress=null;
		...
		public ReportComplex(bool hasGridLines,bool isLandscape,bool showProgress=true) {
			if(showProgress) {
				_actionCloseReportProgress=ODProgressOld.ShowProgressStatus("ReportComplexEvent"
					,typeof(ReportComplexEvent)
					,null
					,hasHistory : PrefC.GetBool(PrefName.ReportsShowHistory)
					,tag : Lan.g("ReportComplex","Running Report Query..."));
			}
		...
		public void AddTitle(string name,string title,Font font){
			ReportComplexEvent.Fire(new ODEventArgs("ReportComplexEvent",Lan.g("ReportComplex","Adding Title To Report")+"..."));
			...
			for(int i=0;i<_reportObjects.Count;i++) {
				if(_reportObjects[i].ObjectType==ReportObjectType.QueryObject) {
					...
					if(!String.IsNullOrWhiteSpace(query.ColumnNameToSplitOn)) { 
						ReportComplexEvent.Fire(new ODEventArgs("ReportComplexEvent",Lan.g("ReportComplex","Creating Splits Based On")+" "+query.ColumnNameToSplitOn+"..."));
