	...
	public class ContrPerio : System.Windows.Forms.Control{
		...
		*/
		...
		public void SaveCurExam(PerioExam PerioExamCur) {
			...
			if(!HasChanged.OfType<bool>().Contains(true)) {
			...
			for(int seqI=0;seqI<PerioMeasures.List.GetLength(1);seqI++){
				for(int toothI=1;toothI<PerioMeasures.List.GetLength(2);toothI++){
					...
					{
						//new measurement
						if(PerioMeasures.List[selectedExam,seqI,toothI]==null || PerioExamCur.IsNew) {//.PerioMeasureNum==0){
						...
						if(seqI==(int)PerioSequenceType.Mobility && PerioMeasureCur.ToothValue==-1 && !PerioExamCur.IsNew) {
							PerioMeasures.Delete(PerioMeasureCur);//-1 is an invalid value for mobility, keep Db bloat down
						}
						else if(PerioMeasures.List[selectedExam,seqI,toothI]==null || PerioExamCur.IsNew) {
		...
		public PerioCell GetCurrentCell() {
			...
			PerioCell curPerioCell=DataArray[CurCell.X,CurCell.Y];
			if(CurCell.Y>=12 && CurCell.Y<=30) {
				curPerioCell.Surface=PerioSurface.Lingual;
			}
			else {
				curPerioCell.Surface=PerioSurface.Facial;
			}
			if(CurCell.Y<=20) {
				curPerioCell.ToothNum=(CurCell.X+2)/3;
			}
			else {
				curPerioCell.ToothNum=33-(CurCell.X+2)/3;
			}
			if(curPerioCell.Surface==PerioSurface.Facial) {
				curPerioCell.ProbingPosition=(CurCell.X-1)%3+1;
			}
			else {
				curPerioCell.ProbingPosition=3-(CurCell.X-1)%3;
			}
			return curPerioCell;
		}
			...
			while(limit<400){//the 400 limit is just a last resort. Should always break manually.
				...
				if((!DirectionIsRight && isReverse) || (DirectionIsRight && !isReverse)){
						...
						|| section==3 && skippedTeeth.Count==32) //skipped all teeth and are on last row
					{
						...
						else if(section==3 && CurCell.X==1) {
							return;//stop on first tooth
						}
						else{
