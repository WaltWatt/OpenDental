...
namespace OpenDental{
	public class FormEmailJobs : System.Windows.Forms.Form {
		...
		private UI.ODGrid gridRecipients;
		...
		private void FormEmailJobs_Load(object sender, System.EventArgs e) {
			...
			_listFeatureReqs=FeatureRequests.GetMany(frNums);
		...
		private void FillGridRecipients() {
			...
			gridRecipients.Columns.Add(new ODGridColumn("Pat\r\nNum",50));
			gridRecipients.Columns.Add(new ODGridColumn("Pat\r\nName",200));
			gridRecipients.Columns.Add(new ODGridColumn("Email",170));
			gridRecipients.Columns.Add(new ODGridColumn("Phone",120));
			gridRecipients.Columns.Add(new ODGridColumn("Quote",50) { TextAlign=HorizontalAlignment.Center });
			gridRecipients.Columns.Add(new ODGridColumn("Task",50) { TextAlign=HorizontalAlignment.Center });
			gridRecipients.Columns.Add(new ODGridColumn("Feature",50) { TextAlign=HorizontalAlignment.Center });
			gridRecipients.Columns.Add(new ODGridColumn("Pledge\r\nAmount",75) { TextAlign=HorizontalAlignment.Right });
			gridRecipients.Columns.Add(new ODGridColumn("Votes",50) { TextAlign=HorizontalAlignment.Right });
			gridRecipients.Columns.Add(new ODGridColumn("Send",50) { TextAlign=HorizontalAlignment.Center });
			gridRecipients.Columns.Add(new ODGridColumn("Status",50) { TextAlign=HorizontalAlignment.Center });
			gridRecipients.Rows.Clear();
			foreach(Jobs.JobEmail jobEmail in _listJobEmails) {
				...
				if(jobEmail.IsSend) {
					row.Cells.Add(new ODGridCell("X") { CellColor=Color.FromArgb(236,255,236) });//light green;
				}
				else {
					row.Cells.Add(new ODGridCell("") { CellColor=Color.FromArgb(222,222,222) });//light gray;
				}
		...
		private void butSend_Click(object sender, System.EventArgs e) {
			...
			if(!string.IsNullOrEmpty(sb.ToString())) {
				MsgBoxCopyPaste msgBox = new MsgBoxCopyPaste("The following errors occured while sending emails:\r\n"+sb.ToString());
				msgBox.ShowDialog();
			}
			FillGridRecipients();
		}
		...
		private void editTemplateToolStripMenuItem_Click(object sender,EventArgs e) {
			FormEmailJobTemplateEdit FormEJTE = new FormEmailJobTemplateEdit();
			FormEJTE.ShowDialog();
