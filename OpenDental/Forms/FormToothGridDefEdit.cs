...
namespace OpenDental{
	...
	public class FormToothGridDefEdit:ODForm {
		...
		private void FillListCellType() {
			foreach(string type in Enum.GetNames(typeof(ToothGridCellType))){
				listCellType.Items.Add(type);
			}
		}

		private void FillListProcStatus() {
			foreach(string type in Enum.GetNames(typeof(ProcStat))) {
				listProcStatus.Items.Add(type);
			}
		}
