...
namespace OpenDental {
	public partial class FormDefinitionPicker:ODForm {
		...
		private void FillGrid() {
			...
			foreach(Def defCur in _listDefs) {
				...
				if(ListSelectedDefs.Any(x => defCur.DefNum==x.DefNum)) {//after adding row, see if it should be reselected after grid update
					listSelectedIndexes.Add(gridMain.Rows.Count-1);//if reselecting this def, add the row count-1 to list of indexes
				}
			}
			gridMain.EndUpdate();
		...
		private void gridMain_CellClick(object sender,ODGridClickEventArgs e) {
			...
			ListSelectedDefs.AddRange(gridMain.SelectedIndices.Select(x => (Def)gridMain.Rows[x].Tag));
		}
