...
namespace OpenDental {
	public partial class FormBugSubmissions:ODForm {
		...
		double CalculateSimilarity(string source,string target) {
			...
			source=Regex.Replace(source,"line [0-9]+",string.Empty);
			target=Regex.Replace(target,"line [0-9]+",string.Empty);
			if((source.Length==0) || (target.Length==0)
				|| !source.Length.Between(target.Length-25,target.Length+25))
			{
				return 0;
			}
