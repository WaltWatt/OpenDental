...
namespace OpenDental {
	public partial class FormEvaluationEdit:ODForm {
		...
		private void gridCriterion_CellLeave(object sender,ODGridClickEventArgs e) {
			...
			if(gridCriterion.SelectedCell.X>=gridCriterion.Rows[gridCriterion.SelectedCell.Y].Cells.Count) {
				return;
			}
			if(_listEvalCrits[gridCriterion.SelectedCell.Y].IsCategoryName) {//This must happen first so categories are always set to blank columns
				gridCriterion.Rows[gridCriterion.SelectedCell.Y].Cells[gridCriterion.SelectedCell.X].Text="";
				return;
			}
			if(gridCriterion.SelectedCell.X==4) {//Cell 4 is the Notes Column
				_listEvalCrits[gridCriterion.SelectedCell.Y].Notes=gridCriterion.Rows[gridCriterion.SelectedCell.Y].Cells[4].Text;
				return;
			}
			//Everything below this point assumes changes may have been made to the GradeNumber column.
			float number=0;
			if(!float.TryParse(gridCriterion.Rows[gridCriterion.SelectedCell.Y].Cells[3].Text,out number)) {// Cell 3 is the GradeNumber column
			...
			if(_dictCritGradeScales[_listEvalCrits[gridCriterion.SelectedCell.Y].GradingScaleNum].ScaleType==EnumScaleType.PickList) {
				...
				for(int i=0;i<_dictCritGradeItems[_listEvalCrits[gridCriterion.SelectedCell.Y].GradingScaleNum].Count;i++) {
					if(number==_dictCritGradeItems[_listEvalCrits[gridCriterion.SelectedCell.Y].GradingScaleNum][i].GradeNumber) {
						...
						gridCriterion.Rows[gridCriterion.SelectedCell.Y].Cells[2].Text=_dictCritGradeItems[_listEvalCrits[gridCriterion.SelectedCell.Y].GradingScaleNum][i].GradeShowing;
						isValid=true;
				...
				gridCriterion.Rows[gridCriterion.SelectedCell.Y].Cells[2].Text=number.ToString();
			}
		...
		private void butOK_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<_listEvalCrits.Count;i++) {
				_listEvalCrits[i].Notes=gridCriterion.Rows[i].Cells[4].Text;//Cell 4 is the notes column
