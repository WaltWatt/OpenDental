...
namespace OpenDental {
	public partial class FormFieldDefLink:ODForm {
		...
		private void FillGridDisplayed() {
			...
			gridDisplayed.Columns.Add(new ODGridColumn("",0));
			gridDisplayed.Rows.Clear();
			ODGridRow row;
			...
			if(gridDisplayed.Rows.Count-1>=selectedIdx) {
		...
		private void butLeft_Click(object sender,EventArgs e) {
			...
			_listFieldDefLinks.Remove((FieldDefLink)gridHidden.Rows[gridHidden.GetSelectedIndex()].Tag);
			FillGridDisplayed();
			FillGridHidden();
		}
