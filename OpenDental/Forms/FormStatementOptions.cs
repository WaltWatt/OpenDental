...
namespace OpenDental{
	...
	public class FormStatementOptions : ODForm {
		...
		private void InitializeComponent()
		{
			...
			this.groupFuchs.Location = new System.Drawing.Point(573, 339);
			...
			// butPatPortal
			// 
			this.butPatPortal.AdjustImageLocation = new System.Drawing.Point(0, 0);
			this.butPatPortal.Autosize = true;
			this.butPatPortal.BtnShape = OpenDental.UI.enumType.BtnShape.Rectangle;
			this.butPatPortal.BtnStyle = OpenDental.UI.enumType.XPStyle.Silver;
			this.butPatPortal.CornerRadius = 4F;
		...
		private void FormStatementOptions_Load(object sender, System.EventArgs e) {
			if(StmtList==null){
				...
				if(StmtCur.StatementType!=StmtType.LimitedStatement && PrefC.HasSuperStatementsEnabled) {//ShowFeatureSuperfamilies && StatementsUseSheets
				...
				if(!StmtCur.SmsSendStatus.In(AutoCommStatus.DoNotSend,AutoCommStatus.Undefined)) {
			...
			#region Bulk Edit
				...
				if(allSame) {
					...
					checkSendSms.Checked=(!StmtList[0].SmsSendStatus.In(AutoCommStatus.DoNotSend,AutoCommStatus.Undefined));
				}
			}
			#endregion Bulk Edit
		...
		private void butPrintClassic() {
				...
				&& !StmtCur.IsInvoice)//Invoices are always recreated on the fly in order to show "Copy" when needed.
			{
				...
				DataSet dataSet=AccountModules.GetStatementDataSet(StmtCur,doIncludePatLName:checkShowLName.Checked);
				string tempFile=FormST.CreateStatementPdfClassic(StmtCur,pat,fam,dataSet);
				try {
		...
		private void butPrintSheets() {
				...
				&& !StmtCur.IsInvoice)//Invoices are always recreated on the fly in order to show "Copy" when needed.
			{
				...
				if(checkSuperStatement.Checked && guarantor!=null && StmtCur.SuperFamily!=0) {
					List<Patient> listFamilyGuarantors=Patients.GetSuperFamilyGuarantors(StmtCur.SuperFamily).FindAll(x => x.HasSuperBilling);
		...
		private bool SaveAsDocument(bool printSheet=false,string pdfFileName="",Sheet sheet=null) {
			...
			if(pdfFileName=="" || printSheet) {
				if(checkSuperStatement.Checked) {
					...
					dataSet=AccountModules.GetSuperFamAccount(StmtCur,doIncludePatLName:checkShowLName.Checked);
				}
				else {
		...
		private void butEmailClassic() {
			if(StmtCur.DocNum!=0 && checkIsSent.Checked) {
				...
				//So create an archive
				if(!CreatePdfClassic() || !CreateEmailMessage()) {
					Cursor=Cursors.Default;
		...
		private void LaunchArchivedPdf(Patient patCur) {
			string patFolder=ImageStore.GetPatientFolder(patCur,ImageStore.GetPreferredAtoZpath());
			string fileName=ImageStore.GetFilePath(Documents.GetByNum(StmtCur.DocNum),patFolder);
			if(!FileAtoZ.Exists(fileName)) {
				MessageBox.Show(Lan.g(this,"File not found:")+" "+Documents.GetByNum(StmtCur.DocNum).FileName);
				return;
			}
			...
			if(StmtList==null){
				...
				for(int i=0;i<StmtList.Count;i++){
					...
					if(checkSendSms.CheckState!=CheckState.Indeterminate) {
						if(checkSendSms.Checked) {
							if(StmtList[i].SmsSendStatus.In(AutoCommStatus.DoNotSend,AutoCommStatus.Undefined)) {
		...
		private void butCancel_Click(object sender, System.EventArgs e) {
			if(StmtList==null && StmtCur.IsNew && StmtCur.IsInvoice) {
				Statements.Delete(StmtCur.StatementNum);
			}
			DialogResult=DialogResult.Cancel;
		}
