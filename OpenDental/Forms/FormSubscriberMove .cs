...
namespace OpenDental {
	public partial class FormSubscriberMove:ODForm {
		...
		private void butOK_Click(object sender,EventArgs e) {
			if(_fromInsPlan==null || InsPlans.GetPlan(_fromInsPlan.PlanNum,new List<InsPlan>())==null) {
				MsgBox.Show(this,"Please pick a valid carrier to move subscribers from.");
				return;
			}
			if(_intoInsPlan==null || InsPlans.GetPlan(_intoInsPlan.PlanNum,new List<InsPlan>())==null) {
				MsgBox.Show(this,"Please pick a valid carrier to move subscribers to.");
				return;
			}
			...
			if(!MsgBox.Show(this,MsgBoxButtons.OKCancel,"Moving subscribers is irreversible.  Always make a full backup before moving subscribers.  "
				+"Patient specific benefits, subscriber notes, benefit notes, and effective dates will not be copied to the \"Ins plan to move subscribers to\"."
