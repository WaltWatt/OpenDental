...
namespace OpenDental {
	public partial class FormApptTypeEdit:ODForm {
		...
		private void FormApptTypeEdit_Load(object sender,EventArgs e) {
			...
			if(AppointmentTypeCur.Pattern!=null) { //logic copied from FormApptEdit
				for(int i=0;i<AppointmentTypeCur.Pattern.Length;i++) {
					_strBTime.Append(AppointmentTypeCur.Pattern.Substring(i,1));
		...
		private void FillTime() {
			...
			if(_strBTime.Length>0) {
				textTime.Text=(_strBTime.Length*ApptDrawing.MinPerIncr).ToString();
			}
			else {
