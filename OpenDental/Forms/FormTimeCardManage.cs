...
namespace OpenDental {
	public partial class FormTimeCardManage:ODForm {
		...
		private void butPrintGrid_Click(object sender,EventArgs e) {
			...
			PrintDocument pd=new PrintDocument();
			pd.PrintPage += new PrintPageEventHandler(this.pd_PrintPage);//<<-- LOOK HERE (pd_PrintPage) to see how pages are printed. (spoiler: it goes to ODGrid.PrintPage)
				...
				if(PrefC.HasClinicsEnabled) {
					...
					if(Security.CurUser.ClinicIsRestricted) {
						...
						if(comboClinic.SelectedIndex==0) {//All is selected.
							...
							text+=Clinics.GetAbbr(_listClinics[comboClinic.SelectedIndex-2].ClinicNum); ;//Subtract 2, because All and Headquarters are in the list.
						}
					}
				}
