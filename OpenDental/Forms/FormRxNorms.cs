	...
	public partial class FormRxNorms:ODForm {
		...
		private void FormRxNorms_Load(object sender,EventArgs e) {
			...
			if(!String.IsNullOrWhiteSpace(InitSearchCodeOrDescript)) {
				...
				if(gridMain.Rows.Count==0) {//If no exact matches, then show similar matches.
					FillGrid(false);
				}
			}
		}
		...
		private void FillGrid(bool isExact) {
			...
			col=new ODGridColumn(Lan.g("FormRxNorms","InMedList"),60,HorizontalAlignment.Center);
			gridMain.Columns.Add(col);
			col=new ODGridColumn(Lan.g("FormRxNorms","MedCount"),60,HorizontalAlignment.Center);
			gridMain.Columns.Add(col);
			col=new ODGridColumn(Lan.g("FormRxNorms","Description"),0);
			gridMain.Columns.Add(col);
			gridMain.Rows.Clear();
			ODGridRow row;
