...
///<summary></summary>
	public class FormRpStatement : ODForm {
		...
		///folders, then it will simply result in a docnum of zero, so no attached doc.  Returns the path of the
		///temp file where the pdf is saved which will be blank if the pdf was not saved successfully. Temp file should be deleted manually.</summary>
		public string CreateStatementPdfClassic(Statement stmt,Patient pat,Family fam,DataSet dataSet){
			Stmt=stmt;
			dataSett=dataSet;
			...
			//Save to a temp pdf--------------------------------------------------------------------------
			string tempPath=CodeBase.ODFileUtils.CombinePaths(PrefC.GetTempFolderPath(),pat2.PatNum.ToString()+".pdf");
		...
		///temp file where the pdf is saved which will be blank if the pdf was not saved successfully. Temp file should be deleted manually.</summary>
		public string CreateStatementPdfSheets(Statement stmt,Patient pat,Family fam,DataSet dataSet) {
