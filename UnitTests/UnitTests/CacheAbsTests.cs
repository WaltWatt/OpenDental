...
namespace UnitTests {
	...
	public class CacheAbsTests:TestBase {
		...
		public void CacheAbsTests_ListTest_GetFindIndexShort() {
			...
			int indexHiddenProv=_listCache.GetFindIndex(x => x.Abbr=="Cache5" && x.FName=="Emma" && x.LName=="Johnson",true);
			...
			//I'm not sure if the following check is going to always be the same, but theoretically they will NOT match in a synchronous environment.
			//If GetProvsNoCache() ever orders its return value OR if the database gives the providers back in random orders, this might fail.
			Assert.IsFalse(index > -1 && indexCache > -1 && index==indexCache);
		}
