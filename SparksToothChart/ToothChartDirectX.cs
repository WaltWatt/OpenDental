...
namespace SparksToothChart {
	...
	public partial class ToothChartDirectX:Control {
		...
		private SharpDX.Color specular_color_normal;
		private SharpDX.Color specular_color_cementum;
		private float specularSharpness;
		private SharpDX.Direct3D9.Font xfont;
		private SharpDX.Direct3D9.Font xSealantFont;
		private SharpDX.Direct3D9.Font xWatchFont;
		private SharpDX.Direct3D9.Font xWatchFontBig;
		...
		public class DirectXDeviceFormat{
			...
			public PresentParameters GetPresentParameters(Control control) {
				...
				pp.BackBufferFormat=(Format)Enum.Parse(typeof(Format),BackBufferFormat);
				...
				pp.MultiSampleType=(MultisampleType)MultiSampleCount;
			...
			public Device CreateDevice(Control control) {				
				...
				return new Device(_d3d,adapterIndex,IsHardware?DeviceType.Hardware:DeviceType.Reference,control.Handle,
		...
		private void PrepareForDirectX() {
			...
			xfont=new SharpDX.Direct3D9.Font(device,
			...
			xSealantFont=new SharpDX.Direct3D9.Font(device,
				...
				FontQuality.ClearType,FontPitchAndFamily.Default,"Arial");
			xWatchFont=new SharpDX.Direct3D9.Font(device,
				(int)Math.Round(15*TcData.PixelScaleRatio),
				(int)Math.Round(6*TcData.PixelScaleRatio),
				FontWeight.Regular,1,false,FontCharacterSet.Ansi,FontPrecision.Device,
				FontQuality.ClearType,FontPitchAndFamily.Default,"Arial");
			xWatchFontBig=new SharpDX.Direct3D9.Font(device,
		...
		///<summary>The isUsingPen flag is only used for regular tooth chart rendering, not for perio charting.
		///If isUsingPen is true, then the scene is not cleared and the teeth are not redrawn.  The tooth rendering is the slowest
		///part of the scene rendering, and the pen drawings are always above the teeth in the scene.  For some customers with Windows 8 and Windows 10,
		///the scene draws as slow as 1 frame per second, which causes the pen drawing to look very jagged if we redraw the teeth.</summary>
		protected void Render(bool isUsingPen=false) {
			...
			specular_color_normal=new SharpDX.Color(specNorm);
			specular_color_cementum=new SharpDX.Color(specCem);
		...
		///<summary>If isUsingPen is true, then the scene is not cleared and the teeth are not redrawn.  The tooth rendering is the slowest
		///part of the scene rendering, and the pen drawings are always above the teeth in the scene.  For some customers with Windows 8 and Windows 10,
		///the scene draws as slow as 1 frame per second, which causes the pen drawing to look very jagged if we redraw the teeth.</summary>
		...
		public static void DrawColoredPolygon(Device device,Color color,params SharpDX.Vector3[] arrayPoints){
			...
			///<summary>The arrayPoints must contain at least 3 points and must contain a point count which is divisible by 3,
		///otherwise no triangles will be drawn.</summary>
		...
		private SizeF MeasureStringMm(string text) {
			...
			SharpDX.Mathematics.Interop.RawRectangle rectSize=xfont.MeasureText(null,text,FontDrawFlags.VerticalCenter);
