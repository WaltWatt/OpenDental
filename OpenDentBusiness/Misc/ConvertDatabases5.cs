...
namespace OpenDentBusiness {
	public partial class ConvertDatabases {
		...
		public static void UpdateSheetFieldDefFieldNameForSheetType(string fieldNameOld,string fieldNameNew,params int[] arraySheetTypes) {
...
No Action Required in many cases, check your new patient Web Sched on your web site to see behavior. See online manual for details.";
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.3.1 - Changing userquery | QueryText to mediumtext"));//No translation in convert script.
			if(DataConnection.DBtype==DatabaseType.MySql) {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.3.1"));//No translation in convert script.
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.3.1 - Adding fee.ClinicNum index"));//No translation in convert script.
			try {
				if(DataConnection.DBtype==DatabaseType.MySql) {
			...
			//ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.3.1 - Increase text to mediumtext"));//No translation in convert script.
			//if(DataConnection.DBtype==DatabaseType.MySql) {
		...
		private static void To17_3_8() {
			string command;
			DataTable table;
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.3.8 - Adding claimproc.DateSuppReceived index"));//No translation in convert script.
			try {
				if(DataConnection.DBtype==DatabaseType.MySql) {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.3.8 - Adding procedurelog.DateComplete index"));//No translation in convert script.
			try {
				if(DataConnection.DBtype==DatabaseType.MySql) {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.3.8 - Adding patient.SecDateEntry index"));//No translation in convert script.
			try {
				if(DataConnection.DBtype==DatabaseType.MySql) {
		...
		private static void To17_3_28() {
			...
			if(DataConnection.DBtype==DatabaseType.MySql && table.Rows.Count > 0 && PIn.Bool(table.Rows[0]["ValueString"].ToString())) {
				//Get every single potential clone in the database.
				ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.3.28 - Clones | Finding Potentials"));
				//Our current clone system requires the following fields to be exactly the same: Guarantor,LName,FName, and Birthdate.
				//We have to join on the patientlink table because users could have manually associated patients to their clones / added new clones entirely.
				//The previous time that we tried to convert old clones into the new system did not account for non-alphabetic characters (symbols) in names.
				command=@"SELECT Guarantor,LName,FName,Birthdate
				...
				//Now that we have all potential clones from the database, go try and find the corresponding master.
				for(int i=0;i<table.Rows.Count;i++) {
					ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.3.28 - Clones | Considering "+(i+1)+" / "+table.Rows.Count));
					command="SELECT PatNum,LName,FName,Birthdate "
						+"FROM patient "
						+"WHERE PatStatus=0 "//PatientStatus.Patient
						+"AND LName='"+POut.String(table.Rows[i]["LName"].ToString())+"' "
				...
				//Set the progress bar back to the way it was.
				ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.3.28"));
			}
		}
		...
		private static void To17_3_32() {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.3.32 - CPOE Radiology Procs Index"));//No translation in convert script.
			try {
				if(DataConnection.DBtype==DatabaseType.MySql) {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.3.32"));//No translation in convert script.
		}
		...
		private static void To17_3_44() {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.3.44"));//No translation in convert script.
		}
		...
		private static void To17_4_1() {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.4.1 - Adding AptNum to emailmessage"));//No translation in convert script.
			if(DataConnection.DBtype==DatabaseType.MySql) {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.4.1"));//No translation in convert script.
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.4.1 - add histappointment.Priority"));
			if(DataConnection.DBtype==DatabaseType.MySql) {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.4.1"));
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.4.1 - add securitylog.DateTPrevious"));
			if(DataConnection.DBtype==DatabaseType.MySql) {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.4.1 - tasksubscription index"));
			try {
				if(DataConnection.DBtype==DatabaseType.MySql) {
		...
		private static void To17_4_36() {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.4.36 - Updating wikipage"));//No translation in convert script.
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.4.36 - Updating wikipagehist"));//No translation in convert script.
		...
		private static void To17_4_52() {
			string command;
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.4.52 - alter histappointment.ProcsColored and Note"));
			if(DataConnection.DBtype==DatabaseType.MySql) {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 17.4.52"));
			if(DataConnection.DBtype==DatabaseType.MySql) {
		...
		private static void To18_1_1() {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 18.1.1 | statement columns"));//No translation in convert script.
			if(DataConnection.DBtype==DatabaseType.MySql) {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 18.1.1"));//No translation in convert script.
			if(DataConnection.DBtype==DatabaseType.MySql) {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 18.1.1 - Updating journalentry"));//No translation in convert script.
			if(DataConnection.DBtype==DatabaseType.MySql) {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 18.1.1 - Updating transaction"));//No translation in convert script.
			if(DataConnection.DBtype==DatabaseType.MySql) {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 18.1.1 - add index on securitylog.UserNum"));
			if(DataConnection.DBtype==DatabaseType.MySql) {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 18.1.1"));
