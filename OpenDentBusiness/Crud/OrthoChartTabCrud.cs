	...
	public class OrthoChartTabCrud {
		...
		public static long Insert(OrthoChartTab orthoChartTab){
			if(DataConnection.DBtype==DatabaseType.Oracle) {
				orthoChartTab.OrthoChartTabNum=DbHelper.GetNextOracleKey("orthocharttab","OrthoChartTabNum");
				int loopcount=0;
				while(loopcount<100){
					try {
						return Insert(orthoChartTab,true);
					}
					catch(Oracle.ManagedDataAccess.Client.OracleException ex){
						if(ex.Number==1 && ex.Message.ToLower().Contains("unique constraint") && ex.Message.ToLower().Contains("violated")){
							orthoChartTab.OrthoChartTabNum++;
							loopcount++;
						}
						else{
							throw ex;
		...
		public static long InsertNoCache(OrthoChartTab orthoChartTab){
			if(DataConnection.DBtype==DatabaseType.MySql) {
				...
				if(DataConnection.DBtype==DatabaseType.Oracle) {
					orthoChartTab.OrthoChartTabNum=DbHelper.GetNextOracleKey("orthocharttab","OrthoChartTabNum"); //Cacheless method
				}
				return InsertNoCache(orthoChartTab,true);
			}
		}
