	...
	public class ClinicErxCrud {
		...
		public static long Insert(ClinicErx clinicErx){
			if(DataConnection.DBtype==DatabaseType.Oracle) {
				clinicErx.ClinicErxNum=DbHelper.GetNextOracleKey("clinicerx","ClinicErxNum");
				int loopcount=0;
				while(loopcount<100){
					try {
						return Insert(clinicErx,true);
					}
					catch(Oracle.ManagedDataAccess.Client.OracleException ex){
						if(ex.Number==1 && ex.Message.ToLower().Contains("unique constraint") && ex.Message.ToLower().Contains("violated")){
							clinicErx.ClinicErxNum++;
							loopcount++;
						}
						else{
							throw ex;
		...
		public static long InsertNoCache(ClinicErx clinicErx){
			if(DataConnection.DBtype==DatabaseType.MySql) {
				...
				if(DataConnection.DBtype==DatabaseType.Oracle) {
					clinicErx.ClinicErxNum=DbHelper.GetNextOracleKey("clinicerx","ClinicErxNum"); //Cacheless method
				}
				return InsertNoCache(clinicErx,true);
			}
		}
