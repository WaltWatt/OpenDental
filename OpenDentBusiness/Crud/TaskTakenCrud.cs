	...
	public class TaskTakenCrud {
		...
		public static long Insert(TaskTaken taskTaken){
			if(DataConnection.DBtype==DatabaseType.Oracle) {
				taskTaken.TaskTakenNum=DbHelper.GetNextOracleKey("tasktaken","TaskTakenNum");
				int loopcount=0;
				while(loopcount<100){
					try {
						return Insert(taskTaken,true);
					}
					catch(Oracle.ManagedDataAccess.Client.OracleException ex){
						if(ex.Number==1 && ex.Message.ToLower().Contains("unique constraint") && ex.Message.ToLower().Contains("violated")){
							taskTaken.TaskTakenNum++;
							loopcount++;
						}
						else{
							throw ex;
		...
		public static long InsertNoCache(TaskTaken taskTaken){
			if(DataConnection.DBtype==DatabaseType.MySql) {
				...
				if(DataConnection.DBtype==DatabaseType.Oracle) {
					taskTaken.TaskTakenNum=DbHelper.GetNextOracleKey("tasktaken","TaskTakenNum"); //Cacheless method
				}
				return InsertNoCache(taskTaken,true);
			}
		}
