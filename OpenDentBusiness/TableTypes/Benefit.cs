	...
	public class Benefit:TableBase, IComparable {
		...
		public int CompareTo(object obj) {
			...
			if(CodeNum!=ben.CodeNum) {
				...
				int frequencyGroup1=0;
				int frequencyGroup2=0;
				if(ProcedureCodes.ListBWCodeNums.Contains(CodeNum)) {
					frequencyGroup1=1;
				}
				else if(ProcedureCodes.ListExamCodeNums.Contains(CodeNum)) {
					frequencyGroup1=2;
				}
				else if(ProcedureCodes.ListPanoFMXCodeNums.Contains(CodeNum)) {
					frequencyGroup1=3;
				}
				if(ProcedureCodes.ListBWCodeNums.Contains(ben.CodeNum)) {
					frequencyGroup2=1;
				}
				else if(ProcedureCodes.ListExamCodeNums.Contains(ben.CodeNum)) {
					frequencyGroup2=2;
				}
				else if(ProcedureCodes.ListPanoFMXCodeNums.Contains(ben.CodeNum)) {
					frequencyGroup2=3;
				}
