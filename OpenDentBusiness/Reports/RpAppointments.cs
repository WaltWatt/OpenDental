...
namespace OpenDentBusiness {
	public class RpAppointments {
			...
			List<long> listConfirmationStatuses,string formSender) 
		{
			...
			if(sortBy==SortAndFilterBy.SecDateEntry) {
				command+="appointment.SecDateEntry,";
			}
			if(sortBy==SortAndFilterBy.SecurityLogDate) {
			...
			command += 
				...
				appointment.SecDateEntry AS 'DateTimeCreated',
				...
				+whereConfStatus;
			if(sortBy==SortAndFilterBy.SecDateEntry) {
				command+=" appointment.SecDateEntry BETWEEN "+POut.Date(dateStart)+" AND "+POut.Date(dateEnd.AddDays(1))
				+" ORDER BY appointment.ClinicNum,appointment.SecDateEntry,PatName";
			}
		...
		public enum SortAndFilterBy {
			///<summary>Sort and filter on appointment.SecDateEntry</summary>
			SecDateEntry,
