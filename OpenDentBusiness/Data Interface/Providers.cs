...
namespace OpenDentBusiness{
	...
	public class Providers {
	  ...
	  ///<summary></summary>
		...
		public static List<Provider> GetProvsByProvNums(List<long> listProvNums) {
			//No need to check RemotingRole; no call to db.
			List<Provider> listProviders=new List<Provider>();
			foreach(long provNum in listProvNums) {
				Provider provider=GetProv(provNum);
				if(provider!=null) {
		...
		public static Dictionary<long,decimal> GetProductionGoalForProviders(List<long> listProvNums,DateTime start,DateTime end) {
			//No need to check RemotingRole; no call to db.
			Dictionary<long,double> dictProvSchedHrs=Schedules.GetHoursSchedForProvsInRange(listProvNums,start,end);
