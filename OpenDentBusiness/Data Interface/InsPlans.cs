...
namespace OpenDentBusiness {
	...
	public class InsPlans {
		...
		///<summary>Pass in the old InsPlan to avoid querying the db for it.
		///No need to pass in usernum, it is set before the remoting role and passed in for logging.</summary>
		public static void Update(InsPlan plan,InsPlan planOld=null,long userNum=0) {
			if(RemotingClient.RemotingRole!=RemotingRole.ServerWeb) {
				userNum = Security.CurUser.UserNum;
			}
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),plan,planOld,userNum);
				return;
			}
			if(planOld==null) {
			...
			InsEditLogs.MakeLogEntry(plan,planOld,InsEditLogType.InsPlan,userNum);
		}
		...
		public static List<InsPlan> GetPlans(List<long> planNums) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<InsPlan>>(MethodBase.GetCurrentMethod(),planNums);
			}
			string command="SELECT * FROM insplan WHERE PlanNum IN ("+string.Join(",",planNums)+")";
			...
			string trojanID,bool showHidden,bool isIncludeAll)
		{
			...
			foreach(DataRow rawRow in rawRows) {
				...
				row["noSendElect"]=(rawRow["NoSendElect"].ToString()=="0"?"":"X");
		...
		///<summary>Based on the four supplied parameters, it updates all similar plans.  Used in a specific tool: FormFeesForIns.
		///No need to pass in UserNum.</summary>
		public static long SetFeeSched(long employerNum,string carrierName,string groupNum,
			string groupName,long feeSchedNum, FeeScheduleType feeSchedType, long userNum=0)
		{
			if(RemotingClient.RemotingRole!=RemotingRole.ServerWeb) {
				userNum=Security.CurUser.UserNum;
			}
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetLong(MethodBase.GetCurrentMethod(),employerNum,carrierName,groupNum,groupName,feeSchedNum,feeSchedType,userNum);
			}
			...
			//table[i][0] = PlanNum
			//table[i][1] = FeeSchedNum (FeeSched, CopayFeeSched, or AllowedFeeSched)
			//table[i][2] = GroupNum 
			//table[i][3] = GroupName
			DataTable table=Db.GetTable(command);
			if(table.Rows.Count==0){
			...
			for(int i=0;i<table.Rows.Count;i++){
				...
				InsEditLogs.MakeLogEntry(POut.String(table.Columns[1].ToString()),
					userNum,POut.String(table.Rows[i][1].ToString()),feeSchedNum.ToString(),
		...
		public static long GenerateAllowedFeeSchedules(long userNum=0) {
			...
			for(int i=0;i<table.Rows.Count;i++){
				...
				listInsPlans.ForEach(x => {
					InsEditLogs.MakeLogEntry("AllowedFeeSched",userNum,"0",POut.Long(sched.FeeSchedNum),InsEditLogType.InsPlan,
		...
		public static void UnusedHideAll(long userNum=0) {
			if(RemotingClient.RemotingRole!=RemotingRole.ServerWeb) {
				userNum=Security.CurUser.UserNum;//must be before normal remoting role check to get user at workstation
			}
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
			...
			listInsPlans.ForEach(x => {
				InsEditLogs.MakeLogEntry("IsHidden",userNum,"0","1",InsEditLogType.InsPlan,
					x.PlanNum,0,x.GroupNum.ToString()+" - "+x.GroupName.ToString());
			});
		}
		...
		///Then it will use the fee schedule for the provider.</summary>
			...
			,long provNum,long clinicNum)
		{
			...
			if(planType=="p" || planType=="f") {
				...
				if(codeNum==substCodeNum) {
					...
					allowedNoSub=Fees.GetAmount(codeNum,feeSched,clinicNum,provNum);
				}
		...
		public static decimal GetAllowedForProc(Procedure proc,ClaimProc claimProc,List<InsPlan> listInsPlans,List<SubstitutionLink> listSubLinks) {
			...
			if(carrierAllowedAmount==-1) {
				...
				return (decimal)(proc.ProcFee*Math.Max(1,proc.BaseUnits+proc.UnitQty));
			}
			else {
				return carrierAllowedAmount*Math.Max(1,proc.BaseUnits+proc.UnitQty);
			}
		}
		...
		public static void Delete(InsPlan plan,long userNum=0,bool canDeleteInsSub=true) {
			if(RemotingClient.RemotingRole!=RemotingRole.ServerWeb) {
				userNum=Security.CurUser.UserNum;//must be before normal remoting role check to get user at workstation
			}
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),plan,userNum,canDeleteInsSub);
				return;
			}
			#region Validation
			//first, check claims
			...
			listBenefits.ForEach(x => {
				InsEditLogs.MakeLogEntry(null,x,InsEditLogType.Benefit,userNum); //log benefit deletion
			});
			...
			Db.NonQ(command);
			InsEditLogs.MakeLogEntry(null,plan,InsEditLogType.InsPlan,userNum); //log insplan deletion
		...
		public static void ChangeReferences(long planNum,long planNumTo,long userNum=0) {
			if(RemotingClient.RemotingRole!=RemotingRole.ServerWeb) {
				userNum=Security.CurUser.UserNum;//must be before normal remoting role check to get user at workstation
			}
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),planNum,planNumTo,userNum);
				return;
			}
			string command;
		...
		public static long SetAllPlansToShowUCR(long userNum=0) {
			if(RemotingClient.RemotingRole!=RemotingRole.ServerWeb) {
				userNum=Security.CurUser.UserNum;//must be before normal remoting role check to get user at workstation
			}
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
			...
			listInsPlans.ForEach(x => { //log insplan ClaimsUseUCR change.
				InsEditLogs.MakeLogEntry("ClaimsUseUCR",userNum,"0","1",InsEditLogType.InsPlan,
		...
		///<summary>No need to pass in userNum, it's set before remoting role check and passed to the server
		///if necessary.</summary>
		public static void UpdateCobRuleForAll(EnumCobRule cobRule, long userNum=0) {
			if(RemotingClient.RemotingRole!=RemotingRole.ServerWeb) {
				userNum=Security.CurUser.UserNum;//must be before normal remoting role check to get user at workstation
			}
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),cobRule,userNum);
				return;
			}
			...
			Db.NonQ(command);
			listInsPlans.ForEach(x => {
				InsEditLogs.MakeLogEntry("CobRule",userNum,x.CobRule.ToString(),POut.Int((int)cobRule),
