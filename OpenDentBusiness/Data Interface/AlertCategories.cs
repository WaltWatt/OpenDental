...
namespace OpenDentBusiness{
	...
	public class AlertCategories{
		...
		#region Get Methods
		///<summary></summary>
		public static List<AlertCategory> Refresh(long patNum){
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<AlertCategory>>(MethodBase.GetCurrentMethod(),patNum);
			}
			string command="SELECT * FROM alertcategory WHERE PatNum = "+POut.Long(patNum);
			return Crud.AlertCategoryCrud.SelectMany(command);
		}
