...
namespace OpenDentBusiness{
	...
	public class Tasks {
		...
		{			
			...
			List<long> listTaskNums=ReportsComplex.RunFuncOnReportServer(() => GetTasksNumsForSearch(userNum,listTaskListNums,taskNum,dateCreatedFrom,
			...
			List<Task> listTasks=ReportsComplex.RunFuncOnReportServer(() => Tasks.GetMany(listTaskNums),doRunOnReportServer)//All tasks for the notes and tasks
			...
			if(doIncludeTaskNote) {
				...
				listTaskNotes=ReportsComplex.RunFuncOnReportServer(() => TaskNotes.RefreshForTasks(listTaskNums),doRunOnReportServer);
			}
		...
		public static List<long> GetTasksNumsForSearch(long userNum,List<long> listTaskListNums,long taskNum,DateTime dateCreatedFrom,
		...
		{
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<long>>(MethodBase.GetCurrentMethod(),userNum,listTaskListNums,taskNum,dateCreatedFrom,dateCreatedTo,
		...
		public static List<Task> RefreshPatientTickets(long patNum) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<Task>>(MethodBase.GetCurrentMethod(),patNum);
			}
			string command="SELECT task.*, "
				+"(SELECT COUNT(*) FROM taskunread WHERE task.TaskNum=taskunread.TaskNum "
		...
		public static Task CopyReminderToNextDueDate(Task reminderTask) {
			...
			TaskUnreads.AddUnreads(newTaskNum,Security.CurUser.UserNum);  //We need the new copy to marked as unread for everyone for when it is "due"
