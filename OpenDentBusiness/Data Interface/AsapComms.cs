...
namespace OpenDentBusiness{
	...
	public class AsapComms {
		...
		public class ApptAvailabilityChecker {
			...
			private List<Tuple<DateTime,long>> _listDateOps;

			public ApptAvailabilityChecker() {
				_listAppts=new List<Appointment>();
				_listDateOps=new List<Tuple<DateTime,long>>();
			}
			...
			public ApptAvailabilityChecker(List<Tuple<DateTime,long>> listDateOps) {
			...
			public bool IsApptSlotAvailable(int minutes,long opNum,DateTime slotStart,DateTime slotEnd) {
				if(!_listDateOps.Any(x => x.Item1==slotStart.Date && x.Item2==opNum)) {
					_listAppts.AddRange(Appointments.GetApptsForDatesOps(new List<Tuple<DateTime,long>> { new Tuple<DateTime,long>(slotStart.Date,opNum) }));
