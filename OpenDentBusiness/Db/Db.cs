...
namespace OpenDentBusiness {
	...
	public class Db {
		...
		private static DataTable GetTableWithRemotingRoleCheck(string command) {
			...
			return ExecuteQueryFunc<DataTable>(() => { 
		...
		internal static string GetCount(string command) {
			...
			return ExecuteQueryFunc<string>(() => { 
		...
		private static T ExecuteQueryFunc<T>(Func<T> f) {
			try {
				return f();
			}
			catch(Exception ex) {
				if(ex.Message.ToLower().Contains("fatal error")) {
					throw new ODException("Query Execution Error",LastCommand,ex);
				}
				throw ex;
			}
		}
