	...
	public class ChartModules {
		...
		public static DataTable GetProgNotes(long patNum,	bool isAuditMode,ChartModuleComponentsToLoad componentsToLoad=null) {
			...
			if(componentsToLoad.ShowCommLog) {//TODO: refine to use show Family
				...
				for(int i=0;i<rawComm.Rows.Count;i++) {
					...
					row["description"]=txt+Lans.g("ChartModule","Comm - ")
						+Defs.GetName(DefCat.CommLogTypes,PIn.Long(rawComm.Rows[i]["CommType"].ToString()));
					row["DocNum"]=0;
					row["dx"]="";
					row["Dx"]="";
					row["EmailMessageNum"]=0;
			...
			if(componentsToLoad.ShowTasks) {
				#region Task
				command="SELECT task.*,tasklist.Descript ListDisc,p1.FName,p1.PatNum "
				...
				+"UNION ALL "
				+"SELECT task.*,tasklist.Descript ListDisc,patient.FName,patient.PatNum "
				+"FROM task "
				...
				for(int i=0;i<rawTask.Rows.Count;i++) {
					...
					row["orionIsOnCall"]="";
					row["orionStatus2"]="";
					row["PatNum"]=rawTaskRow["KeyNum"].ToString();
					row["Priority"]="";
					row["priority"]="";
					row["ProcCode"]="";
			...
			if(componentsToLoad.ShowEmail) {
				...
				command="SELECT EmailMessageNum,MsgDateTime,Subject,BodyText,PatNum,SentOrReceived,UserNum,emailmessage.HideIn "
				+"FROM emailmessage "
