...
namespace OpenDentBusiness.Eclaims {
	public class CanadianOutput {
		...
		public static long SendElegibility(Clearinghouse clearinghouseClin,long patNum,InsPlan plan,DateTime date,Relat relat,string patID,bool doPrint,
			...
			try {
				result=Canadian.PassToIca(strb.ToString(),clearinghouseClin,isAutomatic);
			}
		...
		public static List<Etrans> GetOutstandingTransactions(Clearinghouse clearinghouseClin,bool version2,bool sendToItrans,Carrier carrier,
			Provider prov,bool isAutomatic,PrintCdaClaimForm printCdaClaimForm,PrintCCD printCCD) 
		{
			List<Etrans> etransAcks=new List<Etrans>();
			if(clearinghouseClin==null) {
				throw new ApplicationException("Canadian clearinghouse not found.");
			...
			do {
				...
				if(version2 || sendToItrans) {
					etrans=Etranss.CreateCanadianOutput(0,0,0,clearinghouseClin.HqClearinghouseNum,EtransType.RequestOutstand_CA,0,0,Security.CurUser.UserNum);
				}
				else {
					...
					if(!exit) {
						...
						if(claim!=null) {
							if(etransAck.AckCode=="A") {
								...
								if(!isAutomatic && printCdaClaimForm != null) {
								...
								&& InsSubs.GetOne(claim.InsSubNum).AssignBen)
							{
								...
								Canadian.EOBImportHelper(fieldInputter,listClaimProcsForClaim,listAllProcs,listAllClaimProcs,claim,true,null,clearinghouseClin.IsEraDownloadAllowed);
				...
				if(!exit && !isAutomatic && printCCD != null) {
					try {
						printCCD(etrans,result,true);//Physically print the form.
					}
					catch {
