...
//Warning: this path is not handled properly if trailing slash is missing:
			...
			CanadianNetwork network=CanadianNetworks.GetNetwork(carrierReceiver.CanadianNetworkNum);
			...
			if(planNum2!=0) {
				...
				strb.Append(carrier2.ElectID);//already validated as 6 digit number.  Must always be the secondary carrier, even for secondary (COB) eclaims.
				if(carrierReceiver.CDAnetVersion=="02") {
...
#endif
				result=PassToIca(strb.ToString(),clearinghouseClin,isAutomatic);
			}
			...
			else if(insSub.AssignBen && (clearinghouseClin.IsEraDownloadAllowed!=EraBehaviors.None) 
				&& (fieldInputter.MsgType=="21" || fieldInputter.MsgType=="23")) { //EOB and Predetermination EOB
			...
			//2. The EOB does not contain an embedded EOB, indicating that a COB claim needs to be created and sent.  This is done here, automatically.
			//UI already prevents the automatic creation of the secondary claim when New Claim is clicked in Account for Canada.
			//Clicking New Claim from the Account is the same as using the New Claim dropdown to choose the "Primary" option.
			//User can manually create secondary claims using the New Claim dropdown "Secondary" option.
				...
				if(embeddedLength=="" || embeddedLength=="0000") {//no embedded message
					...
					for(int i=0;i<claimProcsClaim.Count;i++) {//loop through the procs from claim 1
						...
						claimProcsClaim2[i]=Procedures.GetClaimProcEstimate(procNum,claimProcList,insPlan2,insSubNum2);
					}
					...
					Claims.CalculateAndUpdate(procListAll,planList,claim2,patPlansForPatient,benefitList,patient.Age,subList);
		...
		///Currently only applies to EOB and Predetermination EOBs. listLabProcs and listClaimProcs are used to pull information regarding lab procedures.</summary>
		public static void EOBImportHelper(CCDFieldInputter fieldInputter,List<ClaimProc> listClaimProcsForClaim,List<Procedure> listLabProcs,
			...
			foreach(ClaimProc claimProcCur in listClaimProcsForClaim) {
				...
				List<long> listLabProcNums=listLabProcs.FindAll(x => claimProcCur.ProcNum!=0 && x.ProcNumLab==claimProcCur.ProcNum).Select(x => x.ProcNum).ToList();
				...
				if(eobBehavior==EraBehaviors.DownloadAndReceive) {//Preauth claimproc retain the "Preauth" status.
				...
				foreach(CCDField field in dictProcDataClaim[claimProcCur.LineNumber]) {
					...
					switch(field.fieldId) {
						...
						case "G15"://Benefit Amount for the Procedure
							if(isPreEob) {
								claimProcCur.InsPayEst=PIn.Double(RawMoneyStrToDisplayMoney(field.valuestr));
							}
							else {
						...
						case "G58"://Benefit Amount for Lab Proc #1
							if(claimProcLabOne!=null) {
								if(isPreEob) {
									claimProcLabOne.InsPayEst=PIn.Double(RawMoneyStrToDisplayMoney(field.valuestr));
						...
						case "G61"://Benefit Amount for Lab Proc #2
							if(claimProcLabTwo!=null) {
								if(isPreEob) {
									claimProcLabTwo.InsPayEst=PIn.Double(RawMoneyStrToDisplayMoney(field.valuestr));
									//The following 2 commented out lines should not be necessary as they were taken care of before calling this function.
									//ClaimProcs.CanadianLabBaseEstHelper(...) ensures that lab procs and parent procs have the same status.
									//claimProcLabOne.Status=ClaimProcStatus.Preauth;
								}
			...
			if(PIn.Int(fieldInputter.GetFieldById("G10").valuestr,false)!=0) {//Number of Carrier Issues Procedure Codes
				//Carrier Issued Procedure Codes
				List<List<CCDField>> listCarrierProcs=fieldInputter.GetCarrierIssuedProcs();
				foreach(List<CCDField> listFieldsCur in listCarrierProcs) {//Keys are G18 line numbers.
					...
					foreach(CCDField field in listFieldsCur) {
						...
						switch(field.fieldId) {
							...
							case "G21"://Deductible
								if(isPreEob) {
									cpByTotal.DedEst=PIn.Double(RawMoneyStrToDisplayMoney(field.valuestr));
								}
								else {
									cpByTotal.DedApplied=PIn.Double(RawMoneyStrToDisplayMoney(field.valuestr));
								}
								break;
							case "G22"://Eligible percentage
								cpByTotal.Percentage=PIn.Int(RawPercentToDisplayPercent(field.valuestr));
								break;
							case "G23"://Benefit Amount
								if(isPreEob) {
									cpByTotal.InsPayEst=PIn.Double(RawMoneyStrToDisplayMoney(field.valuestr));
								}
								else {
									cpByTotal.InsPayAmt=PIn.Double(RawMoneyStrToDisplayMoney(field.valuestr));
								}
								break;
					...
					#endregion
					ClaimProcs.Insert(cpByTotal);
					listClaimProcsForClaim.Add(cpByTotal);
				}
			}
		...
		public static string PassToIca(string msgText,Clearinghouse clearinghouseClin,bool isAutomatic) { //called from Canadian and CanadianOutput. clinic-level clearinghouse passed in.
			if(clearinghouseClin==null){
				throw new ApplicationException(Lans.g("Canadian","A CDAnet compatible clearinghouse could not be found."));
			}
			string saveFolder=clearinghouseClin.ExportPath;
			if(!Directory.Exists(saveFolder)) {
				throw new ApplicationException(saveFolder+" not found.");
			...
			//		if(isItrans) {
			//			throw new ApplicationException(Lans.g("Canadian","The office sequence number in the response from ITRANS is invalid. Response")+": "+result);
			//		}
			//		else if(isClaimstream) {
			//			throw new ApplicationException(Lans.g("Canadian","The office sequence number in the response from Claimstream is invalid. Response")+": "+result);
			//		}
			//		//Other clearinghouses, if we ever support them.
			//		throw new ApplicationException(Lans.g("Canadian","The office sequence number in the response from the clearinghouse is invalid. Response")+": "+result);
			//	}
			//}
		...
		///<summary>Examines the errorCode passed in and returns the short error message string and sets the errorDescription corresponding to the code. Works for status codes and internal codes.</summary>
