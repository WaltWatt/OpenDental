...
namespace OpenDentBusiness {
    ...
    private List<X12Segment> _listSegments;
		...
		private void SetAttached(List <Etrans835Attach> listAttached) {
			...
			foreach(Hx835_Claim claim in ListClaimsPaid) {						
				Etrans835Attach attach=listAttached.FirstOrDefault(x => x.ClpSegmentIndex==claim.ClpSegmentIndex);
				if(attach!=null) {
		...
		public List<X12ClaimMatch> GetClaimMatches(List<Hx835_Claim> listUnattached=null) {
			...
			foreach(Hx835_Claim claim in listUnattached) {
				...
				claimMatch.ListProcNums=claim.ListProcs.Where(x => x.ProcNum!=0).Select(x => x.ProcNum).ToList();
		...
		private Hx835_Claim ProcessCLP(int segNum,string npi) {
			...
			foreach(Hx835_Adj adj in retVal.ListClaimAdjustments) {//Sum claim level adjustments.
				if(adj.AdjCode=="PR" && adj.IsDeductible) {
					retVal.PatientDeductAmt+=adj.AdjAmt;
				}
				else if(adj.AdjCode=="PR" && !adj.IsDeductible) {
					retVal.PatientPortionAmt+=adj.AdjAmt;
				}
				else {//Adjustments which are not patient responsibility (writeoffs).
		...
		private Hx835_Proc ProcessSVC(int segNum,DateTime dateClaimServiceStart,DateTime dateClaimServiceEnd) {
			...
			for(int i=0;i<proc.ListProcAdjustments.Count;i++) {
				...
				else {//Procedure adjustments which are not patient responsibility (writeoffs).
	...
	public class Hx835_Claim {
		...
		public bool IsProcessed(List<ClaimProc> listClaimProcs,List<Etrans835Attach> listAttaches) {
			if(ClaimNum==0 || !listAttaches.Any(x => x.EtransNum==Era.EtransSource.EtransNum && x.ClaimNum==ClaimNum && x.ClpSegmentIndex==ClpSegmentIndex)) {
				//By definitaion the user must double click into the claim EOB on an ERA to process it.
				//This would create an Etrans835Attach entry.
				return false;
			}
			List<ClaimProc> listClaimProcsForClaim=listClaimProcs
				.Where(x => x.ClaimNum==ClaimNum && x.Status.In(ClaimProcStatus.Received,ClaimProcStatus.Supplemental,ClaimProcStatus.Preauth))
				.ToList();
			if(listClaimProcsForClaim.Count==0) {
				return false;
			}
			bool isSupplemental=GetIsSupplemental(listAttaches);
			List<Hx835_Proc> listProcsNotMatched=new List<Hx835_Proc>();
			foreach(Hx835_Proc proc in ListProcs) {
				ClaimProc matchedClaimProc=listClaimProcsForClaim.FirstOrDefault(x =>
						//Mimics proc matching in claimPaid.GetPaymentsForClaimProcs(...)
						x.ProcNum!=0 && (//Procedures only, no by total claimprocs
							(x.ProcNum==proc.ProcNum) 
							|| (x.CodeSent==proc.ProcCodeBilled 
								&& (isSupplemental && x.Status==ClaimProcStatus.Supplemental //Is known to be supplemental and we observe past supplemental
									|| !isSupplemental && (decimal)x.FeeBilled==proc.ProcFee && x.Status.In(ClaimProcStatus.Received,ClaimProcStatus.Preauth)))
							&& (decimal)x.DedApplied==proc.DeductibleAmt
							&& (decimal)x.AllowedOverride==proc.AllowedAmt
							&& (decimal)x.InsPayAmt==proc.InsPaid
							&& (decimal)x.WriteOff==proc.WriteoffAmt
						)
					);
				...
				listClaimProcsForClaim.Remove(matchedClaimProc);//So it cannot be matched twice.
			}
			if(listProcsNotMatched.Count>0) {
				...
				bool hasValidByTotal=listClaimProcsForClaim
					.Where(x => x.ProcNum==0)
		...
		private bool GetIsSupplemental(List<Etrans835Attach> listAttaches) {
			if(this.ClaimNum==0) {
